...
def data_karyawan():
    if karyawan:
        print("=== Data Karyawan ===")
        print(tabulate(karyawan, headers="keys", tablefmt="grid"))
    else:
        print("Data karyawan kosong.")

#fungsi filter data karyawan berdasarkan divisi, status karyawan, dan domisili
def filter_data():
    if not karyawan:
        print("Data karyawan kosong.")
        return
    while True:
        ...
        hasil = [item for item in karyawan if item[field].lower() == value.lower()]
        if hasil:
...
def tambah_karyawan():
    ...
    posisi = input("Masukkan Posisi (Contoh : Kepala Divisi): ").title()
...
def update_karyawan():
    ...
    for data in karyawan:
        if data["Nomor Induk"].lower() == nomor_induk.lower():
            print(f"Data ditemukan: {data['Nama Pegawai']}")
